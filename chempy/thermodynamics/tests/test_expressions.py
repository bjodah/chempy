...
def test_custom_gibbs():
    R, T = 8.314, 298.15
    dH, dS = -4e3, 16
    MyGibbs = MassActionEq.from_callback(
        _gibbs,
        parameter_keys=("temperature", "R"),
...
def test_GibbsEqConst__unique_keys():
    ...
    assert (
        abs(
            (gee.eq_const({"temperature": T, "dH1": dH / R, "dS1": dS / R}) - ref) / ref
        )
        < 1e-14
    )
...
def test_GibbsEqConst__nested():
    ...
    assert (
        be.abs(
            (
                gee.eq_const({"energy": 2981.5 * du.J, "temperature": T}, backend=be)
                - ref
            )
            / ref
        )
