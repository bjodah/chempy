...
def test_TPolyRadiolytic__units():
    ...
    _check(Radiolytic(TPoly([273.15*u.K, 1.85e-7*u.mol/u.joule, 1e-9*u.mol/u.joule/u.K])))


def test_TPiewcewisePolyMassAction():
    tp1 = TPoly([0, 10, 0.1])
    tp2 = TPoly([273.15, 37.315, -0.1])
    pwp = MassAction(TPiewcewisePoly.from_polynomials([(0, 273.15), (273.15, 373.15)], [tp1, tp2]))
    r = Reaction({'A': 2, 'B': 1}, {'C': 1}, pwp, {'B': 1})
    res1 = r.rate_expr()({'A': 11, 'B': 13, 'temperature': 198.15})
    ref1 = 11*11*13*29.815
    assert abs((res1-ref1)/ref1) < 1e-14
    res2 = r.rate_expr()({'A': 11, 'B': 13, 'temperature': 298.15})
    ref2 = 11*11*13*(37.315 - 25*0.1)
    assert abs((res2-ref2)/ref2) < 1e-14
    with pytest.raises(ValueError):
        r.rate_expr()({'A': 11, 'B': 13, 'temperature': 398.15})
...
def test_PiecewiseTPolyMassAction__units():
    ...
    pwp = MassAction(TPiewcewisePoly.from_polynomials([
        (0*u.kelvin, 273.15*u.kelvin),
        (273.15*u.kelvin, 373.15*u.kelvin)
    ], [tp1, tp2]))
    r = Reaction({'A': 2, 'B': 1}, {'C': 1}, pwp, {'B': 1})
    res1 = r.rate_expr()({'A': 11*u.molar, 'B': 13*u.molar, 'temperature': 198.15*u.kelvin})
...
def test_PiecewiseTPolyMassAction__sympy():
    ...
    T = sp.Symbol('T')
    r = Reaction({'A': 2, 'B': 1}, {'C': 1}, pwp, {'B': 1})
    res1 = r.rate_expr()({'A': 11, 'B': 13, 'temperature': T}, backend=sp)
...
def test_Log10PiecewiseRTPolyMassAction():
    ...
    k_unit = 1
    ratex = MassAction(Log10PiecewiseRTPoly.from_polynomials(bounds, [p1, p2], [k_unit]))
...
@requires(units_library)
def test_TPiecewise():
    expr0 = TPoly([273.15*u.K, 10/u.molar/u.s, 0.1/u.molar/u.s/u.K])
    expr1 = TPoly([298.15*u.K, 12.5/u.molar/u.s, 0/u.molar/u.s/u.K, 2/u.molar/u.s/u.K**2])
    pw = TPiecewise([273.15*u.K, 298.15*u.K, expr0, 298.15*u.K, 373.15*u.K, expr1])
    r = Reaction({'e-(aq)': 2}, {'H2': 1, 'OH-': 2}, pw, {'H2O': 2})
    res0 = r.rate_expr()({'temperature': 293.15*u.K, 'e-(aq)': 1e-13*u.molar})
    ref0 = 12*1e-26 * u.molar/u.s
    assert allclose(res0, ref0)
    assert not allclose(res0, 2*ref0)

    res1 = r.rate_expr()({'temperature': 300.15*u.K, 'e-(aq)': 2e-13*u.molar})
