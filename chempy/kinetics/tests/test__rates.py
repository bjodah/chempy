...
def test_ShiftedTPoly__units():
    ...
    stp2 = ShiftedTPoly(
        [273.15 * u.kelvin, 5 * u.m, 7 * u.m / u.kelvin, 13 * u.m * u.kelvin ** -2]
    )
...
def test_TPolyMassAction__units():
    ...
    res = p(
        {"A": 11 * u.molar, "B": 13 * u.molar, "temperature": 298.15 * u.K}, reaction=r
    )
    ref = 7 + 0.2 * 25 + 0.03 * 25 ** 2 + 0.004 * 25 ** 3
...
def test_TPolyRadiolytic__units():
    ...
    _check(
        Radiolytic(
            ShiftedTPoly(
                [273.15 * u.K, 1.85e-7 * u.mol / u.joule, 1e-9 * u.mol / u.joule / u.K]
            )
        )
    )
...
def test_PiecewiseTPolyMassAction__units():
    tp1 = TPoly(
        [10 / u.molar ** 2 / u.second, 0.1 / u.molar ** 2 / u.second / u.kelvin]
    )
    tp2 = ShiftedTPoly(
        [
            273.15 * u.kelvin,
            37.315 / u.molar ** 2 / u.second,
            -0.1 / u.molar ** 2 / u.second / u.kelvin,
        ]
    )
    pwp = MassAction(
        TPiecewise([0 * u.kelvin, tp1, 273.15 * u.kelvin, tp2, float("inf") * u.kelvin])
    )
    r = Reaction({"A": 2, "B": 1}, {"C": 1}, pwp, {"B": 1})
    res1 = pwp(
        {"A": 11 * u.molar, "B": 13 * u.molar, "temperature": 198.15 * u.kelvin},
        reaction=r,
    )
...
def test_PiecewiseTPolyMassAction__sympy():
    ...
    ref1 = (
        ...
        * 13
        * sp.Piecewise(
            (10 + 0.1 * T, sp.And(sp.S.Zero <= T, T <= 273.15)),
